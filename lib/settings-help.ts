...
export function buildConfigureHelp(cwd: string, config: GitDelegateConfig | undefined): string {
  ...
  const lines = [
    ...
    "",
    "Optional shorthand (model only, uses session provider):",
    "  diffModel  -> shorthand for diff.model",
    "  logModel   -> shorthand for log.model",
    "  blameModel -> shorthand for blame.model",
    "Nested { provider, model } objects take precedence over shorthand keys.",
    "",
